...
public class KafkaProducerConfig {
    ...
    @Bean
    public ProducerFactory<Long, CommandDto> producerCommandDtoFactory() {
        return new DefaultKafkaProducerFactory<>(producerConfigs());
    }

    @Bean
    public KafkaTemplate<Long, CommandDto> kafkaTemplateCommandDto() {
        KafkaTemplate<Long, CommandDto> template = new KafkaTemplate<>(producerCommandDtoFactory());
        template.setMessageConverter(new StringJsonMessageConverter());
        return template;
    }
}
